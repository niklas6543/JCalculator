...
public class Framecalc extends JFrame {
	...
	private JTextField currentInput = eqInput;
	...
	public Framecalc() {
		...
		eqInput.setEnabled(true);
		eqInput.setEditable(false);
		eqInput.setName("cInput");
		eqInput.addFocusListener(new FocusListener() {
		...
		funcBlock.add(eqInput, gbc_currentInput);
		eqInput.setColumns(10);
			...
			public void actionPerformed(ActionEvent arg0) {
				try{
					if (formula.contains("x")){
						...
						sin = false;
						cos = false;
						HashMap<String, Double> varEq = new HashMap<String, Double>();
						//nicht fertig !! hier könnte der Benutzer den Wert der Variable durch ein Textfeld festlegen
						varEq.put("x", Double.parseDouble(var));
						lblOperation.setText(formula+" = "+Double.toString(calculate(varEq)));
					}else
					{
						paint = false;
		...
		varInput.setEditable(false);
		varInput.addFocusListener(new FocusListener() {
		...
		gbc_txtVarX.gridx = 1;
		gbc_txtVarX.gridy = 5;
		comBlock.add(varInput, gbc_txtVarX);
		varInput.setColumns(10);
